...
self = {
  ...
  track: (data) ->
    if data.post.type == 'page_view'
      ...
      for k, widget of data.response.web_forms
        widget.display_type = widget.display_type.replace '_', '-'
        self.add(widget)
  ...
  add: (widget) ->
    ...
    @load widget.body_html, => 
      ...
      if widget.after_page_scroll && widget.after_page_scroll > 0
  ...
  other_conditions: (widget) ->
    ...
    if @_validate_conditions(widget) && !@_has_widget_rendered(widget.display_type)
  ...
  scroll_condition: ->
    ...
    for k of @delayed_scroll_widgets_ids
      ...
      if typeof widget != 'undefined'
        top_percent = widget.after_page_scroll / 100
  ...
  check_overlap: (widget, callback) ->
    ...
    if !@browser_support_web_push() && widget.body_html.indexOf('mkz(\'webPushSubscribe\')') > -1
      @_log 'Exclude the widget with push if browser do not support', widget.id
      return false
    
    if widget.display_type == 'pop-up'

      # Check the overlap of this widget
      for r_widget_id, r_widget of @widgets
        if r_widget.display_type == widget.display_type && r_widget.render && widget.id != r_widget.id
          has_overlap = true
          break
    
      # Check blocked groups of widgets
      group_name = if widget.on_exit || widget.after_page_scroll then 'active' else 'passive'
  ...
  widget_filter: (widget, properties) ->
    if (properties.id && parseInt(widget.id) == parseInt(properties.id)) || (properties.slug && widget.slug == properties.slug)
      return true
    else
      return false
  ...
  _has_widget_rendered: (type) ->
    ...
    for id of @widgets
      widget = @widgets[id]
      if typeof widget.render != 'undefined' && widget.display_type == type
